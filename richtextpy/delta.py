...
class Delta(object):
    ...
    def push(self, op):
        ...
        if ('delete' in new_op) and ('delete' in last_op):
            self.ops[new_index -
                     1] = {'delete': last_op['delete'] + new_op['delete']}
            return self
        ...
        while self_iter.has_next() or other_iter.has_next():
            if other_iter.peek_type() == 'insert':
                ...
                if 'retain' in other_op:
                    ...
                    attributes = attr_compose(self_op.get('attributes'), other_op.get(
                        'attributes'), True if ('retain' in self_op) else False)
                    if attributes != None:
        ...
        while self_iter.has_next() or other_iter.has_next():
            if self_iter.peek_type() == 'insert' and (priority or other_iter.peek_type() != 'insert'):
                ...
                if 'delete' in self_op:
                    ...
                    delta.retain(length, attr_transform(self_op.get(
                        'attributes'), other_op.get('attributes'), priority))
        ...
        for change in changes:
            ...
            while length > 0:
                ...
                if change_type == diff_match_patch.DIFF_INSERT:
                    ...
                    op_length = min(self_iter.peek_length(),
                                    other_iter.peek_length(), length)
                    self_op = self_iter.next(op_length)
                    other_op = other_iter.next(op_length)
                    if self_op['insert'] == other_op['insert']:
                        delta.retain(op_length, attr_diff(self_op.get(
